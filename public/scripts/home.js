...
fetch(ROUTE_HOME + "fill").then(onResponse).then(onJson);
...
function onJson(json){
        ...
        for(let result of results){
            ...
            form_data.append("title",title.textContent);
            form_data.append('_token',CSRF_TOKEN);
            fetch(ROUTE_HOME +"checkFavorites",{method: 'post', body: form_data, mode: 'no-cors'}).then(ResponseButton).then(function(text){return HandButton(text,like,form)});
...
function AddToFavorites(event){
    ...
    form_data.append('_token',CSRF_TOKEN);
    fetch(ROUTE_HOME + "addFavorites", {method: 'post', body: form_data, mode: 'no-cors'});
    event.preventDefault();
