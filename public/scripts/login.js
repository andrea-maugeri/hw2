...
function checkUser(event){
	if(event.currentTarget.value){
        ...
        form_data.append('user',event.currentTarget.value);
        form_data.append('_token',CSRF_TOKEN);
		const stringa_fetch = ROUTE_SIGNUP + '/checkUser';
		fetch(stringa_fetch,{method: 'post', body: form_data, mode: 'no-cors'}).then(onResponse).then(HandErrore);
	}
}
...
function signup(event){
    ...
    form_data.append('_token',CSRF_TOKEN);
	fetch(ROUTE_SIGNUP, {method: 'post', body: form_data, mode: 'no-cors'}).then(onResponse).then(onSignup);
}
